...
<div class="relative" x-data="{ open: false }">
    <button @click="open = !open" class="flex flex-col items-center space-y-1 hover:scale-105 focus:outline-none">
        <img class="rounded-full h-9 w-9" src="{{ asset('storage/' . ($role->image ?? 'user_image/default.jpg')) }}" alt="User Avatar">
        <p class="text-gray-900 text-lg font-medium">{{ $role->name }}</p>
    </button>                    
    <div x-show="open" @click.outside="open = false" class="absolute right-0 mt-2 w-80 bg-white border rounded-lg shadow-lg z-50">
        <div class="p-4 border-b">
            <div class="flex items-center space-x-4">
                <img src="{{ asset('storage/' . ($role->image ?? 'user_image/default.jpg')) }}" alt="Profile Image" class="w-16 h-16 rounded-full">
                <div>
                    <h2 class="text-lg font-semibold text-gray-800">{{ $role->name }}</h2>
                    <p class="text-sm text-gray-600">{{ $role->email }}</p>
                </div>
            </div>
        </div>
        <nav class="p-2 space-y-1">
            <a href="{{ route(''.$dev.'.dashboard') }}" class="flex items-center p-2 rounded-md hover:bg-gray-100 text-gray-700">
        ...
        <form method="POST" action="{{ route(''.$dev.'.logout') }}" class="p-2 border-t">
            @csrf
